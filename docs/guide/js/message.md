...
## 前言
> 昨天在掘金页面上无操作，突然收到消息（点赞、收藏）类的 =>>右上角。于是乎就在想它是怎么在用户无任何操作，实现的消息推送。于是就有这篇文章。
> 
> 本文主要介绍了：SSE的前后端实现、注意点、踩坑实录。大佬可直接跳到实现部分。
>
> 主要实现了：服务器推送消息给客户端(站内信)，精准推送给某一个用户、系统推送给所有用户。
...
* SSE Server-Sent Events，直译  服务器发送事件。
