...
# 8.0
ALTER USER 'xiaoyi'@'%' IDENTIFIED WITH mysql_native_password BY 'xiaoyi1255Admin';
# 5.7
GRANT ALL PRIVILEGES ON *.* TO 'xiaoyi'@'%' IDENTIFIED BY 'xiaoyi1255' WITH GRANT OPTION;
FLUSH PRIVILEGES;
```
...
#重新加载系统防火墙配置，每次执行完添加或删除端口都要执行这条让它生效
firewall-cmd --reload
